...
st.markdown("<h1 style='text-align: center;'>IMDB 2024 MOVIES</h1>", True)
...
r = st.sidebar.radio(label='choose an option', options=('VISUALIZATION', 'FILTERATION'))

if r == 'FILTERATION':
    st.subheader('filter your movie')
    with st.form(key='filter your movie'):
        Gener = st.selectbox('Gener', ['', 'War', 'Action', 'Biography', 'Adventure', 'Animation'])
        Duration = st.selectbox('Duration', ['', '<=2hrs', '2 - 3hrs', '>=3hrs'])
        Voting = st.selectbox('Voting', ['', '<=100', '100 - 1000', '1000 - 10000', '10000 -100000', '>=100000'])
        Rating = st.selectbox('Rating', ['', '<=5.0', '5.0 - 8.0', '>=8.0'])
    ...
    if submit_button:
        ...
        if Voting:
            if 'Voting' in filtered_data.columns:
                if Voting == '<=100':
                    filtered_data = filtered_data[filtered_data['Voting'] <=100]
                elif Voting == '100 - 1000':
                    filtered_data = filtered_data[(filtered_data['Voting'] >=100) & (filtered_data['Voting'] <=1000)]
                elif Voting == '1000 - 10000':
                    filtered_data = filtered_data[(filtered_data['Voting'] >=1000) & (filtered_data['Voting'] <=10000)]
                elif Voting == '>=10000':
                    filtered_data = filtered_data[filtered_data['Voting'] >=10000]
                elif Voting == '>=100000':
        ...
        if Rating:
            if 'Rating' in filtered_data.columns:
                if Rating == '<=5.0':
                    ...
                    filtered_data = filtered_data[(filtered_data['Rating'] >=5.0) & (filtered_data['Rating'] <=8.0)]
                elif Rating == '>=0.8':
                    filtered_data = filtered_data[(filtered_data['Rating'] >=8.0) & (filtered_data['Rating'] <=10.0)]
            else:
